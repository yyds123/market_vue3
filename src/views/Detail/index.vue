    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          ...
          <Zoom :curImg="curImg"/>
          <!-- 小图列表 -->
          <ImageList :changeImg="changeImg" :nextImg="nextImg" :preImg="preImg" :curImg="curImg"/>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item1 in spuSaleAttrList" :key="item1.id">
                ...
                <dd v-for="item2 in item1.spuSaleAttrValueList" :key="item2.id" :class="{active:item2.isChecked==1}" @click="changeChecked(item2,item1.spuSaleAttrValueList)">
...
export default defineComponent({
  ...
  setup(){
    ...
    const getters = useGetters('detail',['categoryView','skuInfo','spuSaleAttrList'])
    const skuInfo = computed(()=>store.getters['detail/skuInfo'])
    const skuNum = ref(1)//购买数量
    const changeChecked = (item2,arr)=>{
      arr.forEach((item)=>{
        item.isChecked = 0
      })
      item2.isChecked = 1
    ...
    const nextImg = ()=>{
      curImg.value = curImg.value>=skuInfo.value.skuImageList.length-1?curImg.value:++curImg.value
    }
    const preImg = ()=>{
      curImg.value = curImg.value==0?0:--curImg.value
    }
    const changeSkuNum = ()=>{
      let val = skuNum.value*1
      if(isNaN(val)||val<=0){
        skuNum.value = 1
      }
      else {
        skuNum.value = Math.floor(skuNum.value)
      }
    }
    const addShopCar = ()=>{
      try {
        store.dispatch('detail/addShopCar',{skuId:router.currentRoute.value.params.skuid,skuNum:skuNum.value})
    ...
    onMounted(()=>{
      store.dispatch('detail/getDetail',router.currentRoute.value.params.skuid)
    })
    return {
      ...getters,
      changeChecked,
